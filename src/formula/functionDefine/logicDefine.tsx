...
const logicFunctionDefines: FunctionGroup['functions'] = {
  if: {
    label: '如果',
    params: [{ forwardInput: true }, { define: { type: 'boolean' } }, { forwardInput: true }],
    loopAfterParams: 2,
    return: {
      scope: 'customReturn',
      createType: async (getType, ...params) => {
        ...
        for (let i = 1; i < params.length; i += 2) {
          if (i + 1 >= params.length) {
            return {
              pass: false,
              error: createError('functionError', params[i].id, '未匹配当前项的值'),
            }
          }
          ...
          if (!nextParamsType) {
            return {
              pass: false,
              error: createError('functionError', nextParams.id, '未找到参数类型'),
            }
          }
          ...
          if (!deepCompareType(firstType, nextParamsType)) {
            return {
              pass: false,
              error: createError('functionError', nextParams.id, '参数类型不能与第一个参数不同'),
            }
          }
        }
    ...
    description: (
      <FunctionDescription
        define={{ functionKey: 'if', name: '如果', paramsList: ['T', '...(boolean, T)'], result: 'T' }}
        detail="根据表达式的结果, 匹配值, 第一个为默认值, 若所有条件都不符合则返回默认值; T:any"
        examples={[
          { params: ['1', 'true', '3'], result: '3' },
          { params: ['1', 'false', '3'], result: '1' },
          { params: ['"a"', '10 < 5', '"b"', '"123" == 123', '"c"'], result: '"a"' },
        ]}
      />
    ),
  },
  isBlank: {
    ...
    description: (
      <FunctionDescription
        define={{ functionKey: 'isBlank', name: '是否为空', paramsList: ['any'], result: 'boolean' }}
        detail="判断传入的值是否为空"
        examples={[
  ...
  isNotBlank: {
    ...
    description: (
      <FunctionDescription
        define={{ functionKey: 'isNotBlank', name: '是否不为空', paramsList: ['any'], result: 'boolean' }}
        detail="判断传入的值是否不为空"
        examples={[
  ...
  equals: {
    ...
    description: (
      <FunctionDescription
        define={{ functionKey: 'equals', name: '是否相等', paramsList: ['...T'], result: 'boolean' }}
