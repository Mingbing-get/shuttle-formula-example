...
import { SelectGroup, SelectOption } from '../../functionSelect/selectPanel/type'

import { functionWithGroups } from '../../functionDefine'

interface Props {
  onPickFunction?: (fnKey: string, fn: WithLabelFunction<React.ReactNode>) => void
}

export default function FormulaFunction({ onPickFunction }: Props) {
  const [hoverFunction, setHoverFunction] = useState<WithLabelFunction<React.ReactNode>>()
  ...
  return (
    <div className="formula-function-wrapper">
      ...
      />
      {hoverFunction?.description && <div className="formula-function-example">{hoverFunction.description}</div>}
    </div>
  )
}
