...
import type { WithDynamicVariable, WithDynamicVariableObject, GetDynamicObjectByPath } from 'shuttle-formula/render'
...
export interface VariablePickerPanelProps {
  ...
  onSelect?: (path: string[], variable: WithDynamicVariable, variablePath: WithDynamicVariable[]) => void
...
export default function FormulaVariablePickerPanel({
  ...
  }, [variables, variablePath, openPath, showAll, canSelectMinLevel, canDynamicMaxLevel, getDynamicObjectByPath])

  const loadData = useCallback(
    async (option: VariableNode) => {
      if (!isWithDynamicObject(option.variable)) return

      const subVar = await getDynamicObjectByPath?.(option.path, option.variable)
      if (!subVar) return
...
async function variableToTreeNode({
  ...
  for (const key in variables) {
    ...
    if (getDynamicObjectByPath && path.length > 0 && isWithDynamicObject(currentVariable)) {
      const newVariable = await getDynamicObjectByPath(currentPath, currentVariable)
      if (newVariable) {
        currentVariable = newVariable
      }
    }

    if (currentVariable.type !== 'object' || isWithDynamicObject(currentVariable)) {
      nodes.push({
        key,
        title: currentVariable.label ?? key,
        path: currentPath,
        isDisabled: canSelectMinLevel ? currentPath.length < canSelectMinLevel : false,
        variable: currentVariable,
        isLeaf: currentVariable.type !== 'object' || (!!canDynamicMaxLevel && currentPath.length > canDynamicMaxLevel),
      })
      continue
    }
...
function isWithDynamicObject(variable: WithDynamicVariable): variable is WithDynamicVariableObject {
  return variable.type === 'object' && !(variable as any).prototype && (variable as any).dynamic
}
