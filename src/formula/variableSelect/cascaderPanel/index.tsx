...
import { IconRight, IconLoading } from '@arco-design/web-react/icon'
...
export default function CascaderPanel<T extends Record<string, any>>({
  ...
  useRefEffect(
    ...
    (refOpenOnSelectPath?: boolean, refOnOpen?: (path: string[], option?: CascaderOption<T>) => void) => {
  ...
  return (
    <div className={classNames('cascader-panel-container', className)} style={style}>
      <CascaderSinglePanel
        openPath={_openPath}
...
function CascaderSinglePanel<T extends Record<string, any>>({
  ...
  return (
    <>
      <div className={classNames('cascader-panel-wrapper', wrapperClassName)} style={wrapperStyle}>
        {options.map((option) => (
          <div
            ...
            key={option.key}
          >
            <div className="cascader-panel-title" onClick={() => handleSelect([option.key], option)}>
              {option.title}
            </div>

            {loadingMap[option.key] ? (
              <IconLoading className="cascader-panel-loading-icon" spin />
            ) : (
              (option.children?.length || (loadMore && !option.isLeaf)) && (
                <IconRight
                  className="cascader-panel-next-icon"
                  onClick={() => handleOpenNext(openNextOption?.key === option.key ? [] : [option.key], option)}
                />
              )
            )}
          </div>
      ...
      {openNextOption?.children && (
        <CascaderSinglePanel
          ...
          onSelect={(path, option) => handleSelect([openNextOption.key, ...path], option)}
          onOpen={(path, option) => handleOpenNext([openNextOption.key, ...path], option)}
          loadMore={loadMore}
        />
      )}
    </>
