import { WithDynamicVariable, GetDynamicObjectByPath, WithDynamicVariableObject } from 'shuttle-formula/render'
...
export default async function getVariableValueByPath(
  ...
  if (isWithDynamicObject(currentDefine)) {
    const dynamicVariable = await getDynamicObjectByPath?.(leftPath, currentDefine)
  ...
  if (currentDefine.type === 'object') {
    return await getVariableValueByPath(leftPath, currentDefine.prototype, getDynamicObjectByPath, value)
  }
}
