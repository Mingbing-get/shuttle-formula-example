...
import { WithDynamicVariable, GetDynamicObjectByPath } from 'shuttle-formula/render'
...
export default async function computedFormula({ context, contextDefine, code, getDynamicObjectByPath }: Options) {
  if (!code) return
  ...
  calculateExpression.setGetVariableFu(async (path) => {
    return await getVariableValueByPath(path, contextDefine || {}, getDynamicObjectByPath, context)
  })
  calculateExpression.setGetFunctionFu((functionName) => functionValues[functionName])
  ...
  const result = await calculateExpression.execute(ast.syntaxRootIds, ast.syntaxMap)
